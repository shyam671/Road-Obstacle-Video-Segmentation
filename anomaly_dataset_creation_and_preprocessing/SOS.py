...
path = '/shared-local/DATASETS/Video_Anomaly_Segmentation/SOS/'


iters = 0
iters_total = 0

for label_path in Path(path).glob('semantic_ood/**/*.png'):

    iters_total = iters_total + 1
  
    label = np.array(Image.open(label_path))

    label[label == 0] = 1      # Road
    label[label == 254] = 2    # Anomaly
    label[label == 255] = 0    # Void

    save_path_file = str(label_path).split('/')[:-1]
    save_path_file = '/'.join(save_path_file)    
    save_path_file = save_path_file.replace('/shared-local/DATASETS/Video_Anomaly_Segmentation/SOS/','../anomaly_maps/SOS/')
