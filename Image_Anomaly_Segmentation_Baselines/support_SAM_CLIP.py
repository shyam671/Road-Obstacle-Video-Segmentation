...
# SAM2 Model
cs_classes = ['road', 'sidewalk', 'building', 'wall', 'fence', 'pole', 'traffic light', 'traffic sign', 'vegetation', 'terrain', 'sky', 'person', 'rider', 'car', 'truck', 'bus', 'train', 'motorcycle', 'bicycle']
...
text = tokenizer(cs_classes).to(device)
...
class OODEvaluator:
    ...
    def save_anomaly_images(
        ...
        for x, y, path in tqdm(loader, desc="Dataset Iteration"):
            ...
            score = score.cpu().numpy().astype('float16')            
    ...
    def compute_anomaly_scores_from_images(self):
        ...
        print('this is tc', get_common(ood_gts_single_list, score_single_list, 16, th_score.shape[0], th_score.shape[1]))
    ...
    def compute_anomaly_scores(
        ...
        for x, y, path in tqdm(loader, desc="Dataset Iteration"):
            ...
            # For HVC
            th_score_da = np.array(score).copy()
            th_score = np.array(score).copy()
            th_score_da[score > 0.5] = 0
            th_score_da[score < 0.5] = 1    
            th_score_da[y == 255] = 0 # 0 the void area
            y_da = y.copy()
            y_da[y==1] = 255 # Anomaly to background
            y_da[y==0] = 1 # 0-> 1 label of road
            y_da[y==255] = 0 # 255-> 0 non object
            ood_gts_single_list_da.append(y_da)
            score_single_list_da.append(th_score_da)
            th_score[score > 0.5] = 1
            th_score[score < 0.5] = 0
            y[y==255] = 0
            ood_gts_single_list.append(y)
        ...
        VC_a = get_common(ood_gts_single_list, score_single_list, 16, th_score.shape[0], th_score.shape[1])
        VC_da = get_common(ood_gts_single_list_da, score_single_list_da, 16, th_score.shape[0], th_score.shape[1])
        print(VC_a, VC_da)
        print('this is harmonic vc', 2*(VC_a*VC_da)/(VC_da+VC_a))
        
        del ood_gts_single_list, score_single_list, ood_gts_single_list_da, score_single_list_da
            ...
            for root, _, filenames in os.walk(os.path.join(dataset_root, 'Labels', mode)):
                for filename in sorted(filenames):
                    if os.path.splitext(filename)[1] == '.png':
                        ...
                        clip_sam_seg_output = torch.zeros(image.shape[0], image.shape[1])

                        for i in range(0, len(masks)):
                            image_ = image.copy()
                            image_[masks[i]['segmentation']==False] = 0
                            x_, y_, _ = np.where(image_ > 0)
                            image_ = image_[x_.min():x_.max(), y_.min():y_.max(), :]
                            image_ =  Image.fromarray(image_)
                            image_ = preprocess(image_).unsqueeze(0).to(device)
                            
                            with torch.no_grad(), torch.cuda.amp.autocast():
                                image_features = model.encode_image(image_)
                                image_features /= image_features.norm(dim=-1, keepdim=True)
                                text_probs = (100.0 * image_features @ text_features.T).softmax(dim=-1)
                            # For now i am not taking threhold but would be intresting to observe its effect.
                            clip_sam_seg_output[masks[i]['segmentation']==True] = text_probs.max()
                        #score = F.interpolate(clip_sam_seg_output.unsqueeze(0).unsqueeze(0), size=(image.shape[0], image.shape[1]), mode='bilinear', align_corners=False)
                        score = 1-clip_sam_seg_output.squeeze(0).cpu().numpy()
    ...
    def compute_anomaly_scores_SOS_T1(
        self,
        loader,
        device=torch.device('cpu'),
        return_preds=False,
        ...
        for dirname in os.listdir(os.path.join(dataset_root, 'Labels')):
            for filename in sorted(os.listdir(os.path.join(dataset_root, 'Labels',dirname))):
                ...
                for i in range(0, len(masks)):
                    image_ = image.copy()
                    #image_[masks[i]['segmentation']==False] = 0
                    x_, y_ = np.where(masks[i]['segmentation'] > 0)
                    image_ = image_[x_.min():x_.max(), y_.min():y_.max(), :]
                    image_ =  Image.fromarray(image_)
                    image_ = preprocess(image_).unsqueeze(0).to(device)
    ...
    def compute_anomaly_scores_SOS_T2(
        self,
        loader,
        device=torch.device('cpu'),
        return_preds=False,
    ...
    def compute_anomaly_scores_SOS_T3(
            self,
            loader,
            device=torch.device('cpu'),
            return_preds=False,
            ...
            for dirname in os.listdir(os.path.join(dataset_root, 'Labels', mode, 'Label')):
                for camera_number in ['Camera 5', 'Camera 6']:
                    if len(sorted(os.listdir(os.path.join(dataset_root, 'Labels', mode, 'Label', dirname, camera_number)))) > 1000:
                        ...
                        VC = get_common(ood_gts_single_list, score_single_list, 256, th_score.shape[0], th_score.shape[1])
                        sIoU_gt, prec_pred, f1_mean = aggregate(component_level_results)
    ...
    # def temporal_compute_anomaly_scores(
    #     self,
    #     loader,
    #     device=torch.device('cpu'),
    #     return_preds=False,
    ...
    #     counter = 1
    #     moving_metrics = {'sIoU_gt': 0.0, 'prec_pred': 0.0, 'f1_mean': 0.0}
    #     temporal_component_level_results = []
    ...
    #         component_level_results.append(process_frame(np.array(y.squeeze().cpu().numpy()), np.array(score)))   
    #         temporal_component_level_results.append(process_frame(np.array(y.squeeze().cpu().numpy()), np.array(score)))   
            
    #         temporal_frame_count = temporal_frame_count + 1

    #         if temporal_frame_count == temporal_frame:       

    #             sIoU_gt, pred_pred, f1_mean = aggregate(temporal_component_level_results)
                
    #             moving_metrics['sIoU_gt'] = (moving_metrics['sIoU_gt']*(counter-1) + sIoU_gt)/counter
    #             moving_metrics['prec_pred'] = (moving_metrics['prec_pred']*(counter-1) + pred_pred)/counter
    #             moving_metrics['f1_mean'] = (moving_metrics['f1_mean']*(counter-1) + f1_mean)/counter
    #             print(moving_metrics['sIoU_gt'], moving_metrics['prec_pred'], moving_metrics['f1_mean'])
    #             temporal_component_level_results = []
    #             temporal_frame_count = 0
    #             counter = counter + 1
