...
if args.dataset_mode == 'selective':
    ...
    dataset_group = [g for g in dataset_group if g[0] in ['lidarSOD', 'lost_and_found', 'sos']]
    #dataset_group = [g for g in dataset_group if g[0] in ['lidarSOD', 'lost_and_found', 'sos']]
...
def run_evaluations(model, dataset, model_name, dataset_name):
    ...
    #anomaly_score, ood_gts = evaluator.save_anomaly_images(loader=loader, device=DEVICE, return_preds=False, upper_limit= 100000)

    ## anomaly score calculator

    #anomaly_score, ood_gts, tc, tc_a, tc_b, tc_ab = evaluator.compute_anomaly_scores(loader=loader, device=DEVICE, return_preds=False, upper_limit= 100000)
...
def main():
    ...
    file_path = "IAS_baseline_results.txt"
    ...
    for model_name in models_list:
        ...
        if not os.path.exists(model_path):
            model_path = os.path.join(experiment_path, 'model_final.pkl')
        ...
        for dataset_name, dataset in dataset_group:
            ...
            saving_score = open(file_path, append_write)
            saving_score.write(dataset_name + "......." + args.score_func +"......." + str(results[dataset_name]) +'\n')
            saving_score.close()
