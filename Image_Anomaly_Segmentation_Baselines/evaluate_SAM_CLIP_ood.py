...
from support_SAM_CLIP import get_datasets, OODEvaluator
...
if args.dataset_mode == 'selective':
    ...
    dataset_group = [g for g in dataset_group if g[0] in ['sos', 'lost_and_found', 'lidarSOD']]
...
def run_evaluations(model, dataset, model_name, dataset_name):
    ...
    # evaluator.compute_anomaly_scores_SOS_T3(
    #     loader=loader,
    #     device=DEVICE,
    #     return_preds=False,
    #     upper_limit= 10
    ...
    # evaluator.compute_anomaly_scores_SOS_T2(
    #     loader=loader,
    #     device=DEVICE,
    #     return_preds=False,
    #     upper_limit= 10
    ...
    # evaluator.compute_anomaly_scores_SOS_T1(
    #     loader=loader,
    #     device=DEVICE,
    #     return_preds=False,
    #     upper_limit= 10
...
def main():
    ...
    mask_generator_2 = SAM2AutomaticMaskGenerator(model=sam2_model, points_per_side=32, points_per_batch=128, pred_iou_thresh=0.7, stability_score_thresh=0.92, stability_score_offset=0.7, crop_n_layers=1, box_nms_thresh=0.7, crop_n_points_downscale_factor=2, min_mask_region_area=25.0, use_m2m=False,)
    results = edict()
    for dataset_name, dataset in dataset_group:
        if dataset_name not in results:
            results[dataset_name] = edict()
        print(dataset_name)
        results[dataset_name] = run_evaluations(mask_generator_2, dataset, 'SAM2', dataset_name)
        break
