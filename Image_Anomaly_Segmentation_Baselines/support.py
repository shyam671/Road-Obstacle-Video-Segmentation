...
class OODEvaluator:
    ...
    def save_anomaly_images(
        ...
        for x, y, path in tqdm(loader, desc="Dataset Iteration"):
            ...
            im1.save(main_path + 'RbA/' +'_'.join(path[0].split('/')[-2:]))
            # im1 = Image.fromarray(np.uint8(cm.hot(score)*255)).convert('RGB')
    ...
    def compute_anomaly_scores_from_images(self):
        ...
        print('this is tc', get_common(ood_gts_single_list, score_single_list, 5, th_score.shape[0], th_score.shape[1]))
    ...
    def compute_anomaly_scores(
        ...
        for x, y, path in tqdm(loader, desc="Dataset Iteration"):
            ...
            # th_score[score > -0.1] = 1
            # th_score[score < -0.1] = 0
            th_score[score > 0.5] = 1
            th_score[score < 0.5] = 0
            th_score[y==255] = 0
            y[y==255] = 0
        ...
        tc, tc_a, tc_b = get_common(ood_gts_single_list, score_single_list, 7, th_score.shape[0], th_score.shape[1])
        print(tc, tc_a, tc_b, 2*tc_a*tc_b/(tc_a+tc_b))
        del ood_gts_single_list, score_single_list
        # aggregate(component_level_results)
        # ood_gts = np.array(ood_gts)
        # anomaly_score = np.array(anomaly_score)
        ...
        return anomaly_score, ood_gts, tc, tc_a, tc_b, 2*tc_a*tc_b/(tc_a+tc_b)
            ...
            for dirname in os.listdir(os.path.join(dataset_root, 'Labels', mode, 'Label')):
                for camera_number in ['Camera 5', 'Camera 6']:
                    if len(sorted(os.listdir(os.path.join(dataset_root, 'Labels', mode, 'Label', dirname, camera_number)))) > 1000:
                        ...
                        pbar.close()
        print('printing global stats',  global_AuROC/counter, global_AuPRC/counter, global_FPR95/counter, 100*global_VC/counter, 100*global_sIoU_gt/counter, 100*global_prec_pred/counter, 100*global_f1_mean/counter)                    
    ...
    #     return anomaly_score, ood_gts
            ...
            # ood_gts_single_list.append(y)
            # score_single_list.append(th_score)
        # VC_a = get_common(ood_gts_single_list, score_single_list, 3, th_score.shape[0], th_score.shape[1])
        # #print(VC_a, VC_da)
