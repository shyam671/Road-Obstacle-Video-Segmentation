...
def plot_roc(preds, labels, save_path, title="Receiver operating characteristic"):
    ...
    #plt.savefig(save_path + '/Auroc_plot.png')
    return plt
    #plt.show()


def plot_pr(preds, labels, save_path, title="Precision recall curve"):
    ...
    #plt.figure()
    lw = 2
    plt.plot(recall, precision, color='mediumvioletred', lw=lw, label='PRC curve (area = %0.2f)' % prc_auc)
    ...
    plt.title(title)
    plt.legend(loc="lower right")
    plt.savefig(save_path + '/AuPRC_plot.png')
    return plt

    #plt.show()


def plot_barcode(preds, labels, save_path):
    ...
    #plt.savefig(save_path + '/barcode.png')
    return plt
