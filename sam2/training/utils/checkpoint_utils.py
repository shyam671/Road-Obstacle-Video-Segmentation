...
def load_state_dict_into_model_vps(
    state_dict: Dict,
    model: nn.Module,
    strict: bool = False,
    vps_chekpoint_path: str = None,
    ignore_missing_keys: List[str] = None,
    ignore_unexpected_keys: List[str] = None,
    checkpoint_kernels: List[Callable] = None,
):

    #### new way of loading model weights
    #'sam_mask_decoder_cs.mask_tokens', 'sam_mask_decoder_cs.iou_prediction_head']
    sam2_model_weights = torch.load(vps_chekpoint_path)
    new_sam2_model_weights = copy.deepcopy(sam2_model_weights)

    for key, value in sam2_model_weights['model'].items():
        if 'sam_mask_decoder_cs.mask_tokens' in key.replace('sam_mask_decoder','sam_mask_decoder_cs') or 'sam_mask_decoder_cs.iou_prediction_head' in key.replace('sam_mask_decoder','sam_mask_decoder_cs') :
            continue
        if 'sam_mask_decoder' in key:
                new_key = key.replace('sam_mask_decoder','sam_mask_decoder_cs')
                new_sam2_model_weights['model'].update({new_key:value})
    missing_keys, unexpected_keys = model.load_state_dict(new_sam2_model_weights['model'], strict=False)
