...
class VOSDataset_VPS(VisionDataset):
    def __init__(
        self,
        transforms,
        training: bool,
    ...
    def _get_datapoint(self, idx):
        ...
        datapoint = self.construct(video, segment_loader)
        for transform in self._transforms:
            datapoint = transform(datapoint, epoch=self.curr_epoch)

        return datapoint

    def construct(self, video, segment_loader):
        """
        ...
        sampled_frames = video.frames
        images = []
        random_y = randrange(0, 1024)
        crop_area = (0, random_y, 1024, random_y + 1024)

        rgb_images = load_images_random_crop(sampled_frames, crop_area)
        ...
        for frame_idx, frame in enumerate(sampled_frames):
            ...
            segment = segment_loader.load((frame.image_path.split('/')[-1].replace('_leftImg8bit', '_gtFine'), crop_area))
            images[frame_idx].objects.append(Object(object_id=0, frame_index=frame.frame_idx, segment=segment,))
...
def load_images_random_crop(frames, crop_area):
    all_images = []
    cache = {}
    for frame in frames:
        if frame.data is None:
            ...
            if path in cache:
                ...
                continue
            with g_pathmgr.open(path, "rb") as fopen:
                all_images.append(PILImage.open(fopen).convert("RGB").crop(crop_area))
            cache[path] = len(all_images) - 1
        else:
            all_images.append(tensor_2_PIL(frame.data))
