...
class PalettisedPNGSegmentLoader_VPS:
    ...
    def load(self, frame_id):
        ...
        # check the path
        mask_path = os.path.join(self.video_png_root, frame_id[0])
        
        # load the mask
        gt = np.array(PILImage.open(mask_path).convert("L"))
        gt = gt[frame_id[1][0]:frame_id[1][2], frame_id[1][1]:frame_id[1][3]]
        #print(np.unique(np.array(gt)), 'after', frame_id[1], gt.shape)

        classes = pd.unique(gt.flatten())
        classes = classes[classes != 255]  # remove background (255) for cityscapes
        masks = torch.from_numpy(np.array(gt)) 
        return masks
