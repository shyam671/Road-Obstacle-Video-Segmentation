...
from training.dataset.vos_segment_loader import (
    ...
    SA1BSegmentLoader,
    PalettisedPNGSegmentLoader_VPS,
)
...
class PNGRawDataset_VPS_2F(VOSRawDataset):
    def __init__(
        self,
        img_folder,
        gt_folder,
        ...
        self.video_names = sorted(os.listdir(self.gt_folder))        

    def get_video(self, idx):

        if idx + 2 > len(self.video_names):
            idx = idx - 2
            
        all_frames = []
        for index in range(2):
            all_frames.append(os.path.join(self.img_folder, self.video_names[idx + index].replace('_gtFine', '_leftImg8bit')))

        frames = []
        for _, fpath in enumerate(all_frames):
            fid = os.path.basename(fpath).split(".")[0]
            frames.append(VOSFrame(fid, image_path=fpath))
        video = VOSVideo(self.video_names[idx], idx, frames)
        segment_loader = PalettisedPNGSegmentLoader_VPS(self.gt_folder)
