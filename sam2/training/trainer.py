...
from training.utils.checkpoint_utils import (
    ...
    load_state_dict_into_model_vps,
)
...
class Trainer:
    ...
    def load_checkpoint(self):
        ckpt_path = self.checkpoint_conf.model_weight_initializer.state_dict.checkpoint_path
        if ckpt_path is None:
            self._init_model_state()
        else:
            if self.checkpoint_conf.initialize_after_preemption:
                self._call_model_initializer()
            self._load_resuming_checkpoint_vps(ckpt_path)
    ...
    def _load_resuming_checkpoint_vps(self, ckpt_path: str):
